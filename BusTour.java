/*
A bus tour of Europe has been very successful. Due to an increase in the number of people that want to go on a tour, the tour company decided to increase the height of the bus.
...
public class BusTour {
    public static void main(String[] args) {
        ...
        int[] bridgesHeights = new int[bridgesNumber];
        boolean willCrash = false;
        for (int x = 0; x < bridgesNumber; x++){
            bridgesHeights[x] = scanner.nextInt();
        }
        for (int x = 0; x < bridgesNumber; x++){
            if (bridgesHeights[x] <= height) {
                System.out.println("Will crash on bridge " + (x+1));
                willCrash = true;
                break;
            }
        }
        if (willCrash != true) System.out.println("Will not crash");
    }
}
